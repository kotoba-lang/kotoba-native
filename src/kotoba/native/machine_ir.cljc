...
(defn- x86-page-fault-handler-address [dst]
  ;; cr3-h2: cli; CR2/error-code classification; G/W/X debugcon receipt;
  ;; deterministic isa-debug-exit. The bytes are `kotoba.native.interrupt-abi`'s,
  ;; which derives the data page from the GDTR rather than assuming 0x110000.
  (let [n (count interrupt-abi/page-fault-classifier-handler-bytes)]
    (vec (concat [0xe9] (u32le n)
                 interrupt-abi/page-fault-classifier-handler-bytes
...
(defn- x86-rt-timer-handler-address [dst]
  (let [n (count interrupt-abi/rt-timer-handler-bytes)]
    (vec (concat [0xe9] (u32le n)
                 interrupt-abi/rt-timer-handler-bytes
                 [0x4c 0x8d 0x15] (u32le (- (+ n 7)))
                 (when-not (= dst :x86-64/r10)
                   (x86-rr 0x89 dst :x86-64/r10))))))
...
(defn- x86-page-fault-recovery-handler-address [dst]
  (let [n (count interrupt-abi/page-fault-recovery-handler-bytes)]
    (vec (concat [0xe9] (u32le n)
                 interrupt-abi/page-fault-recovery-handler-bytes
                 [0x4c 0x8d 0x15] (u32le (- (+ n 7)))
                 (when-not (= dst :x86-64/r10)
                   (x86-rr 0x89 dst :x86-64/r10))))))
...
(defn- x86-double-fault-handler-address [dst]
  (let [n (count interrupt-abi/double-fault-handler-bytes)]
    (vec (concat [0xe9] (u32le n)
                 interrupt-abi/double-fault-handler-bytes
                 [0x4c 0x8d 0x15] (u32le (- (+ n 7)))
                 (when-not (= dst :x86-64/r10)
                   (x86-rr 0x89 dst :x86-64/r10))))))
  ...
  (let [copy-to (fn [register value]
    ...
    (case action
      ...
      (finish (concat (copy-to :x86-64/r10 a)
                      ...
                      interrupt-abi/configure-page-fault-recovery-bytes)
              :x86-64/r10)
      ...
      (finish (concat (copy-to :x86-64/r10 a)
                      (copy-to :x86-64/r11 b)
                      interrupt-abi/configure-double-fault-ist-bytes)
              :x86-64/r10)
      :load-gdt-tss
      (finish (concat (copy-to :x86-64/r10 a)
                      (copy-to :x86-64/r11 b)
      ...
      :probe-nx-execute
      (finish interrupt-abi/probe-nx-execute-bytes :x86-64/r10)
