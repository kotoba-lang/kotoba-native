...
(def ^:private double-fault-handler-bytes
  ;; TSS.IST1 entry has a 48-byte same-CPL frame (including old SS/RSP) and
  ;; the architectural zero error code. Publish only after exact stack/frame
  ;; validation; #DF is a terminal fail-closed receipt.
...
(def ^:private configure-double-fault-ist-bytes
    ...
    (let [[op & args] form]
      (cond
        ...
        (vec (concat (emit-expr (first args) env (assoc ctx :tail? false)) [0x50]
                     (emit-expr (second args) env (assoc ctx :tail? false))
                     [0x48 0x89 0xc1 0x58 0x49 0x89 0xc2 0x49 0x89 0xcb]
                     configure-double-fault-ist-bytes [0x4c 0x89 0xd0]))
