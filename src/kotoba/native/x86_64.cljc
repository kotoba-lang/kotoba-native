...
;; A keyword is carried as the same one-word `pair(offset,length)` handle a
    ...
    (let [[op & args] form]
      (cond
        ...
        (= op 'kernel-publish-current-domain)
        (vec (concat (emit-expr (first args) env (assoc ctx :tail? false))
                     [0x41 0x89 0x81 0x10 0x01 0x00 0x00]))
        (= op 'kernel-value-runtime-capability-table)
        [0x49 0x8d 0x81 0x00 0x10 0x00 0x00]
        (= op 'kernel-value-provider-queue)
        [0x49 0x8d 0x81 0x00 0x04 0x00 0x00]
        (= op 'kernel-value-runtime-arena)
        [0x49 0x8d 0x81 0x00 0x20 0x00 0x00]
        (= op 'kernel-value-runtime-cas-scratch)
        [0x49 0x8d 0x81 0x00 0x06 0x00 0x00]
        (= op 'kernel-publish-value-provider-status)
        (vec (concat (emit-expr (first args) env (assoc ctx :tail? false))
                     [0x41 0x89 0x81 0x38 0x01 0x00 0x00]))
        (= op 'kernel-value-provider-status)
        [0x41 0x8b 0x81 0x38 0x01 0x00 0x00]
