...
(def ^:private load-gdt-tss-and-readback-bytes
  ;; NOTE: the canned handlers (`interrupt-abi/page-fault-recovery-handler-bytes`
  ;; etc.) derive the kernel context from GDTR.base - `context-gdt-offset` and
  ;; check it against the context's own GDTR copy. A GDT loaded elsewhere with
  ;; this operation makes every canned handler take its fail-closed path.
...
(def ^:private heap-call-offsets
  {'pair 56 'pair-first 64 'pair-second 72
   ...
   ;; Both offsets are past 127 and so take the disp32 call form below, the
   ;; same branch every offset from 128 up already takes.
...
;; A keyword is carried as the same one-word `pair(offset,length)` handle a
    ...
    (let [[op & args] form]
      (cond
        ...
        ;; cr3-h2: the classifier's bytes are `kotoba.native.interrupt-abi`'s,
        ;; which derives the data page from the GDTR instead of assuming it.
        (= op 'kernel-page-fault-handler-address)
        (let [n (count interrupt-abi/page-fault-classifier-handler-bytes)]
          (vec (concat [0xe9] (le32 n) interrupt-abi/page-fault-classifier-handler-bytes
                       [0x48 0x8d 0x05] (le32 (- (+ n 7))))))
        (= op 'kernel-rt-timer-handler-address)
        (let [n (count interrupt-abi/rt-timer-handler-bytes)]
          (vec (concat [0xe9] (le32 n) interrupt-abi/rt-timer-handler-bytes
                       [0x48 0x8d 0x05] (le32 (- (+ n 7))))))
        (= op 'kernel-page-fault-recovery-handler-address)
        (let [n (count interrupt-abi/page-fault-recovery-handler-bytes)]
          (vec (concat [0xe9] (le32 n) interrupt-abi/page-fault-recovery-handler-bytes
                       [0x48 0x8d 0x05] (le32 (- (+ n 7))))))
        ...
        (vec (concat (emit-expr (first args) env (assoc ctx :tail? false)) [0x50]
                     ...
                     interrupt-abi/configure-page-fault-recovery-bytes [0x4c 0x89 0xd0]))
        (= op 'kernel-configure-double-fault-ist)
        (vec (concat (emit-expr (first args) env (assoc ctx :tail? false)) [0x50]
                     (emit-expr (second args) env (assoc ctx :tail? false))
                     [0x48 0x89 0xc1 0x58 0x49 0x89 0xc2 0x49 0x89 0xcb]
                     interrupt-abi/configure-double-fault-ist-bytes [0x4c 0x89 0xd0]))
        (= op 'kernel-load-gdt-tss)
        (vec (concat (emit-expr (first args) env (assoc ctx :tail? false)) [0x50]
                     (emit-expr (second args) env (assoc ctx :tail? false))
                     [0x48 0x89 0xc1 0x58 0x49 0x89 0xc2 0x49 0x89 0xcb]
                     load-gdt-tss-and-readback-bytes [0x4c 0x89 0xd0]))
        (= op 'kernel-load-idt)
        (vec (concat (emit-expr (first args) env (assoc ctx :tail? false))
                     [0x50]
                     (emit-expr (second args) env (assoc ctx :tail? false))
        ...
        ;; cr3-h2: `mov r10,r9; call r10` -- the context register IS the data
        ;; page. The old `movabs r10,0x110000` named text in the aiueos kernel.
        (= op 'kernel-probe-nx-execute)
        (into interrupt-abi/probe-nx-execute-bytes [0x31 0xc0])
        (= op 'kernel-probe-recoverable-guard-write)
        [0x49 0xc7 0xc2 0x00 0x00 0x10 0x00 0x41 0xc6 0x02 0x00
         0x4d 0x31 0xd2]
        (= op 'kernel-probe-double-fault)
...
(defn emit-program [kir]
  (let [kir (-> kir
    ...
    (machine-ir/compile-kir-module
     :x86-64 kir
     (machine-ir/entry-fuel-prefixes
      kir (fn [_] (fuel-charge-tokens (atom -1)))))))
