...
(def ^:private page-fault-handler-bytes
  [0xfa 0x41 0x0f 0x20 0xd2 0x4c 0x8b 0x1c 0x24
   ...
   0x49 0x81 0xfa 0x00 0xb0 0x10 0x00 0x74 0x2c 0xeb 0x3f
...
(def ^:private page-fault-recovery-handler-bytes
  ;; Preserve all registers touched by the sealed handler. The CPU-pushed
  ;; error/RIP frame stays on the interrupted stack while the classification
  ;; and receipt body run on the configured dedicated stack. Only the exact
  ;; eight-byte guard-write probe may advance RIP and iretq.
  [])

(def ^:private configure-page-fault-recovery-bytes
  ;; r10=frame page, r11=stack page. Both must be distinct, nonzero and
  ;; 4-KiB aligned. Publish frame base and a 16-byte-aligned stack top into
  ;; fixed kernel context slots, then read both values back.
  [0x4c 0x89 0xd0 0x4c 0x09 0xd8 0x48 0xa9 0xff 0x0f 0x00 0x00
   0x75 0x42 0x4d 0x85 0xd2 0x74 0x3d 0x4d 0x85 0xdb 0x74 0x38
   0x4d 0x39 0xd3 0x74 0x33 0x4c 0x89 0x14 0x25 0x00 0xb1 0x10 0x00
   0x49 0x8d 0x83 0xf0 0x0f 0x00 0x00 0x48 0x89 0x04 0x25 0x08 0xb1 0x10 0x00
   0x4c 0x3b 0x14 0x25 0x00 0xb1 0x10 0x00 0x75 0x12
   0x48 0x3b 0x04 0x25 0x08 0xb1 0x10 0x00 0x75 0x08
   0x49 0xc7 0xc2 0x01 0x00 0x00 0x00 0xc3 0x0f 0x0b])

(def ^:private load-idt-and-readback-bytes
    ...
    (let [[op & args] form]
      (cond
        ...
        (vec (concat (emit-expr (first args) env (assoc ctx :tail? false)) [0x50]
                     ...
                     [0x48 0x89 0xc1 0x58 0x49 0x89 0xc2 0x49 0x89 0xcb]
                     configure-page-fault-recovery-bytes [0x4c 0x89 0xd0]))
        (= op 'kernel-load-idt)
        (vec (concat (emit-expr (first args) env (assoc ctx :tail? false))
                     [0x50]
                     (emit-expr (second args) env (assoc ctx :tail? false))
                     [0x48 0x89 0xc1 0x58
                      0x49 0x89 0xc2 0x49 0x89 0xcb]
                     load-idt-and-readback-bytes
                     [0x4c 0x89 0xd0]))
        (= op 'kernel-probe-guard-write)
        [0xc6 0x04 0x25 0x00 0x00 0x10 0x00 0x00 0x31 0xc0]
        (= op 'kernel-probe-text-write)
        [0xc6 0x04 0x25 0x00 0x10 0x10 0x00 0x00 0x31 0xc0]
        (= op 'kernel-probe-nx-execute)
        [0x49 0xba 0x00 0xb0 0x10 0x00 0x00 0x00 0x00 0x00
         0x41 0xff 0xd2 0x31 0xc0]
        (= op 'kernel-probe-recoverable-guard-write)
        [0x31 0xc0 0xc6 0x04 0x25 0x00 0x00 0x10 0x00 0x00]
