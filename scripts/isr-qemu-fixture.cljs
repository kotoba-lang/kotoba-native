#!/usr/bin/env nbb
;; isr: boot a Kotoba kernel image whose IDT names a toolchain-generated
;; interrupt entry, and read the console it wrote.
;;
;; This repository does not run compiled programs, so its own suite is
;; encodings only -- `interrupt_abi_test.clj` says the bytes are the
;; instructions they are named for, and nothing there says the CPU agrees.
;; This does. It is a FIXTURE and not a test: it needs amu, QEMU and OVMF,
;; none of which this repo depends on, so it is run by hand and its result is
;; quoted in the ADR rather than gating a suite.
;;
;;   nbb scripts/isr-qemu-fixture.cljs --amu <path-to-amu-worktree>
;;
...
(defn -main [script & argv]
  ...
  (let [root (path/dirname (path/dirname (path/resolve script)))
        ...
        source (or (arg argv "--source")
                   (path/join root "test" "fixtures" "isr-qemu.kotoba"))
        out (or (arg argv "--out")
