...
`kotoba.native.machine-ir` defines the extraction pilot for the missing middle:
a closed GMIR integer/control subset, target selection, virtual registers, a
deterministic minimal allocator, and allocated MC instruction/layout data.
Unknown operations, use-before-definition, and required spills fail closed.
Production expression families move through this seam incrementally; the
module does not claim that arbitrary KIR already uses the pilot.
...
lives at the ownership boundary: `kotoba-kir` provides DefCID and `artifact`
provides SourceCID, BuildCID, and ArtifactCID. This native repository consumes
those contracts rather than defining another codec.
